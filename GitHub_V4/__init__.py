...
class GitHub_v4:
  ...
  def __init__(self, token, db_handle):
    ...
    '''
    # initialize headers
    self.github_v4_cve_headers['Authorization'] = 'token {0}'.format(token)
    self.github_v4_normal_headers['Authorization'] = 'token {0}'.format(token)
  ...
  def write_structured_json(self, file_name, json_obj, file_type):
    ...
    if file_type == 'repo':
      ...
      file_path = 'output/cve/{}'
    with open(file_path.format(file_name), 'wt') as f:
  ...
  def make_graphql_query(self, query, headers):
    ...
    '''
    request = requests.post(self.github_v4_url, json={'query': rate_query}, headers=headers)
    rate_limit = request.json()
    if rate_limit['data']['rateLimit']['remaining'] < 100:
      ...
      d2 = datetime.datetime.strptime(rate_limit['data']['rateLimit']['resetAt'],'%Y-%m-%dT%H:%M:%SZ')
      time.sleep((d2 - d1).seconds + self.sleep_time)

    for count in range(1, self.max_retry_count + 1):
      request = requests.post(self.github_v4_url, json={'query': query}, headers=headers)
  ...
  def get_cves_for_org(self, org):
    ...
    '''
    query = self.cve.get_full_org_cve_info(org)
    ...
    while repo_page_info['hasNextPage']:
      query = self.cve.get_full_org_cve_info(org, repo_page_info['endCursor'])
      pages = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = request['data']['organization']['repositories']['edges'] + pages['data']['organization']['repositories']['edges']
    ...
    for repo in request['data']['organization']['repositories']['edges']:
      if (len(repo['node']['vulnerabilityAlerts']['edges']) > 0):
        ...
        while (cve_page_info['hasNextPage']):
          ...
          extra_CVE = self.get_cve_for_repo(org, repo['node']['name'], cve_page_info)
          repo['node']['vulnerabilityAlerts']['edges'] = edges + extra_CVE['data']['organization']['repository']['vulnerabilityAlerts']['edges']

        repos_with_cve.append(repo)
        self.write_structured_json('{}-{}-CVE-{}.json'.format(org, repo['node']['name'],
                                                              datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S')),
                                    repo, 'cve')
    request['data']['organization']['repositories']['edges'] = repos_with_cve
    return request
  
  def get_cve_for_repo(self, org, repo, cve_page_info = { 'endCursor': None, 'hasNextPage': False }):
    '''
      Get CVEs for a repo and or continue getting them
    '''
    query = self.cve.get_repo_cve_info(org, repo)
    request = self.make_graphql_query(query, self.github_v4_normal_headers)
    cve_page_info = request['data']['organization']['repository']['vulnerabilityAlerts']['pageInfo']
    while (cve_page_info['hasNextPage']):
      # play some cup and ball to get everything
      query = self.cve.get_repo_cve_info(org, repo, cve_page_info['endCursor'])
      pages = self.make_graphql_query(query, self.github_v4_normal_headers)
      edges = request['data']['organization']['repository']['vulnerabilityAlerts']['edges'] + pages['data']['organization']['repository']['vulnerabilityAlerts']['edges']
      request['data']['organization']['repository']['vulnerabilityAlerts']['edges'] = edges
      cve_page_info = pages['data']['organization']['repository']['vulnerabilityAlerts']['pageInfo']
    request['data']['organization']['repository']['vulnerabilityAlerts']['pageInfo'] = cve_page_info
    return request
  ...
  def get_org_all_repo_details(self, org, repo_list):
    ...
    for repo_desc in repo_list:
      self.write_structured_json('{}-{}-{}.json'.format(org, repo_desc['name'],
                                                        datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S')),
                                 self.get_full_repo(org, repo_desc['name']), 'repo')
  ...
  def get_full_repo(self, org, repo):
    ...
    logging.info('Requesting base info for {}/{}'.format(org, repo))
    query = self.repo.get_single_repo_base(org, repo, self.repo.get_repo_base_info())
    request = self.make_graphql_query(query, self.github_v4_normal_headers)
    repo_info = request

    if ((repo_info['data']['organization'] is not None) and (repo_info['data']['organization']['repository'] is not None)):
      # gather additional assignable_users
      assignable_users = repo_info['data']['organization']['repository']['assignableUsers']
      if (assignable_users is not None):
        repo_info['data']['organization']['repository']['assignableUsers'] = self.paginate_assignable_users(org, repo, assignable_users)

      # gather additional branch_protection_rules
      branch_protection_rules = repo_info['data']['organization']['repository']['branchProtectionRules']
      if (branch_protection_rules is not None):
        repo_info['data']['organization']['repository']['branchProtectionRules'] = self.paginate_branch_protection_rules(org, repo, branch_protection_rules)

      # gather additional collaborators
      collaborators = repo_info['data']['organization']['repository']['collaborators']
      if (collaborators is not None):
        repo_info['data']['organization']['repository']['collaborators'] = self.paginate_collaborators(org, repo, collaborators)

      # gather additional issues
      issues = repo_info['data']['organization']['repository']['issues']
      if (issues is not None):
        repo_info['data']['organization']['repository']['issues'] = self.paginate_issues(org, repo, issues)

      # gather additional labels
      labels = repo_info['data']['organization']['repository']['labels']
      if (labels is not None):
        repo_info['data']['organization']['repository']['labels'] = self.paginate_labels(org, repo, labels)

      # gather additional languages
      languages = repo_info['data']['organization']['repository']['languages']
      if (languages is not None):
        repo_info['data']['organization']['repository']['languages'] = self.paginate_languages(org, repo, languages)

      # gather additional milestones
      milestones = repo_info['data']['organization']['repository']['milestones']
      if (milestones is not None):
        repo_info['data']['organization']['repository']['milestones'] = self.paginate_milestones(org, repo, milestones)

      # gather additional projects
      projects = repo_info['data']['organization']['repository']['projects']
      if (projects is not None):
        repo_info['data']['organization']['repository']['projects'] = self.paginate_projects(org, repo, projects)
      ...
      if (releases is not None):
        repo_info['data']['organization']['repository']['releases'] = self.paginate_releases(org, repo, releases)

      # gather additional repository_topics
      repository_topics = repo_info['data']['organization']['repository']['repositoryTopics']
      if (repository_topics is not None):
        repo_info['data']['organization']['repository']['repositoryTopics'] = self.paginate_repository_topics(org, repo, repository_topics)
  ...
  def paginate_assignable_users(self, org, repo, assignable_users):
    ...
    while assignable_users['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_users_min('assignableUsers',
                assignable_users['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = assignable_users['edges'] + request['data']['organization']['repository']['assignableUsers']['edges']
      assignable_users['edges'] = edges
      assignable_users['pageInfo'] = request['data']['organization']['repository']['assignableUsers']['pageInfo']
  ...
  def paginate_branch_protection_rules(self, org, repo, branch_protection_rules):
    ...
    while branch_protection_rules['pageInfo']['hasNextPage']:
      ...
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = branch_protection_rules['edges'] + request['data']['organization']['repository']['branchProtectionRules']['edges']
      branch_protection_rules['edges'] = edges
      branch_protection_rules['pageInfo'] = request['data']['organization']['repository']['branchProtectionRules']['pageInfo']
  ...
  def paginate_collaborators(self, org, repo, collaborators):
    ...
    while collaborators['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_users_min('collaborators',
                collaborators['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = collaborators['edges'] + request['data']['organization']['repository']['collaborators']['edges']
      collaborators['edges'] = edges
      collaborators['pageInfo'] = request['data']['organization']['repository']['collaborators']['pageInfo']
  ...
  def paginate_issues(self, org, repo, issues):
    ...
    while issues['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_issues(issues['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = issues['edges'] + request['data']['organization']['repository']['issues']['edges']
      issues['edges'] = edges
      issues['pageInfo'] = request['data']['organization']['repository']['issues']['pageInfo']
    return issues
  ...
  def paginate_labels(self, org, repo, labels):
    ...
    while labels['pageInfo']['hasNextPage']:
      ...
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = labels['edges'] + request['data']['organization']['repository']['labels']['edges']
      labels['edges'] = edges
      labels['pageInfo'] = request['data']['organization']['repository']['labels']['pageInfo']
    return labels
  ...
  def paginate_languages(self, org, repo, languages):
    ...
    while languages['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_languages(languages['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = languages['edges'] + request['data']['organization']['repository']['languages']['edges']
      languages['edges'] = edges
      languages['pageInfo'] = request['data']['organization']['repository']['languages']['pageInfo']
    return languages
  ...
  def paginate_milestones(self, org, repo, milestones):
    ...
    while milestones['pageInfo']['hasNextPage']:
      ...
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = milestones['edges'] + request['data']['organization']['repository']['milestones']['edges']
      milestones['edges'] = edges
      milestones['pageInfo'] = request['data']['organization']['repository']['milestones']['pageInfo']
    return milestones
  ...
  def paginate_projects(self, org, repo, projects):
    ...
    while projects['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_projects(projects['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = projects['edges'] + request['data']['organization']['repository']['projects']['edges']
      projects['edges'] = edges
      projects['pageInfo'] = request['data']['organization']['repository']['projects']['pageInfo']
    return projects
  ...
  def paginate_pull_requests(self, org, repo):
    ...
    query = self.repo.get_single_repo_base(org, repo, self.repo.part_pull_requests())
    request = self.make_graphql_query(query, self.github_v4_cve_headers)
    ...
    while pull_requests['pageInfo']['hasNextPage']:
      ...
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = pull_requests['edges'] + request['data']['organization']['repository']['pullRequests']['edges']
      pull_requests['edges'] = edges
      pull_requests['pageInfo'] = request['data']['organization']['repository']['pullRequests']['pageInfo']
  ...
  def paginate_releases(self, org, repo, releases):
    ...
    while releases['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_releases(releases['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = releases['edges'] + request['data']['organization'] ['repository']['releases']['edges']
      releases['edges'] = edges
      releases['pageInfo'] = request['data']['organization']['repository'] ['releases']['pageInfo']
    return releases
  ...
  def paginate_repository_topics(self, org, repo, repository_topics):
    ...
    while repository_topics['pageInfo']['hasNextPage']:
      query = self.repo.get_single_repo_base(org, repo,
                self.repo.part_repository_topics(repository_topics['pageInfo']['endCursor']))
      request = self.make_graphql_query(query, self.github_v4_cve_headers)
      edges = repository_topics['edges'] + request['data']['organization']['repository']['repositoryTopics']['edges']
      repository_topics['edges'] = edges
      repository_topics['pageInfo'] = request['data']['organization']['repository']['repositoryTopics']['pageInfo']
